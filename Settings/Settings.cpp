...
int favoriteCategoryFiller(TCHAR options[MAX_OPTIONS][MAX_LOADSTRING],
    ...
    hr = pFolderIItem->GetProps(&rgPropID, CEDB_ALLOWREALLOC, 1, &pVals,
		&cbBuffer, hHeap);
    CHR(hr);

	// Default choice is AUTO [Favorites]
	StringCchPrintf(options[index++], MAX_LOADSTRING, TEXT("%s [%s]"),
    ...
    while (start < end) {
        ...
        hr = StringCchCopyN(options[index++], MAX_LOADSTRING, start,
			comma - start);
        CHR(hr);
        start = comma + 2;
    }
...
int skinFiller(TCHAR options[MAX_OPTIONS][MAX_LOADSTRING], TCHAR * value) {
	...
	if (pstr) *(++pstr) = '\0';
    StringCchCat(szIniPath, MAX_PATH, TEXT("*.png"));
