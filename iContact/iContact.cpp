...
int         GroupPosition[MAX_LETTER_COUNT];
int		    ListItemHover = -1;
...
HWND InitInstance (HINSTANCE hInstance, LPWSTR lpCmdLine, int nCmdShow){
    ...
    nKeyboardLetters = wcslen(pSettings->alphabet);
    SwitchTab(hWnd, 2);
    InitSurface(hWnd);
...
LRESULT DoPaintMain (HWND hWnd, UINT wMsg, WPARAM wParam, 
    ...
    if (bTransitioning) {
        ...
        int nListWidth = rList.right - rList.left;
        int nListHeight = rList.bottom - rList.top;
        
        int nRightWidth = (int)(nListWidth * dTransitionPct);
        if (nTransitionType == T_SLIDE_RIGHT)
            nRightWidth = nListWidth - nRightWidth;

        int nLeftWidth = nListWidth - nRightWidth;

        DrawListOn(hdcTmp, hdcTmp2, rList);
        BitBlt(hdcMem, rList.left, rList.top, nLeftWidth, nListHeight, 
            hdcTmp, nRightWidth, rList.top, SRCCOPY);

        dItem = pListData->GetItem(ListItemSelected);
        DrawItemDetailsOn(hdcTmp, dItem, rList);
        BitBlt(hdcMem, nLeftWidth, rList.top, nRightWidth, nListHeight,
	...
	for (h = 0; h <= nKeyboardRows; h++) {
        ...
        for (g = 0; g < nKeyboardCols; g++) {
            ...
            if (i < nKeyboardLetters) {
                ExtTextOut(hdc, x, y, NULL, NULL, &pSettings->alphabet[i++], 1, 0);
            }
		}
	}
}
...
void InitSurface(HWND hWnd) {
    ...
    if (!hbmSkin) {
	    ...
	    if (pstr) *(++pstr) = '\0';
	    wcscat(szSkinFileName, pSettings->skin_name);
        wcscat(szSkinFileName, L".png");
...
void CalculateHeights() {
	...
	for (int i = 0; i < count; i++) {
        ...
        if (pListData->IsItemNewGroup(i) && pListData->GetItem(i).wcGroup) {
            ...
            if (index >= 0 && index < MAX_LETTER_COUNT) {
    ...
    for (int i = 1; i < MAX_LETTER_COUNT; i++) {
	...
	ListHeight = c;
    AverageItemHeight = count == 0 ? DEFAULT_ITEM_HEIGHT : ListHeight / count;
}
...
void ScrollBar(int y) {
    ...
    if (nCurrentTab == 2) {
        int index = (y - rList.top) * 26 / rListHeight;
        Scrolled = index < 0 ? 0
            : index >= 26 ? nListMax
            : GroupPosition[index];
    }
